...
print('target :', target)
target = np.array(target) # 벡터 연산을 위해 np사용.
...
# 3. 토큰형 : 일차적 전처리 완료된 texts -> token 토큰으로 만듦
tokenizer = Tokenizer(num_words=4000) # 약 8000개 // 그러나 4천개정도만 되어도 스팸인지 아닌지 구별 가능
...
model.compile(optimizer = 'adam',
              loss = "binary_crossentropy", # 이항분류에 쓰이는 것
              metrics=['accuracy']) # 모델의 평가
