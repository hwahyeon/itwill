...
2. 저수준 AIP vs 고수준  API
 - 저수준(low level) API : model, layer 작성이 좀 어렵다.-> code 어렵다. 그러나 원리 이해
 - 고수준(high level) API : model, layer 작성이 쉽다. -> code 쉽다.
...
'''
1차
outputs = [1.25], y_pred = [1.5]
손실값 = 0.0625 -> 미분계수 = [0.625]

2차
outputs = [1.9], y_pred = [1.5]
손실값 = 0.15999998 -> 미분계수 = [-1.5199999]

3차
outputs = [2.8], y_pred = [1.5]
손실값 = 1.6899998 -> 미분계수 = [-7.2799997]


output < y_pred : 미분계수 +값
output > y_pred : 미분계수 -값
'''
