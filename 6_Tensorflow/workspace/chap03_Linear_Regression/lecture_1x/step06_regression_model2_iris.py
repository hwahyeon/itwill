...
tf.disable_v2_behavior() # ver 2.x 사용 안함 
...
# 2. X, Y 변수 정의 : 공급형 변수
X = tf.placeholder(dtype=tf.float32, shape = [None, 3]) # [관측치, 입력값??]
...
opt = tf.train.AdamOptimizer(0.4) # 학습률

train = opt.minimize(loss) # 손실 최소화 식

# 5. model 학습 -> model 최적화(최적의 a와 b가 업데이트 되었다는 뜻)
