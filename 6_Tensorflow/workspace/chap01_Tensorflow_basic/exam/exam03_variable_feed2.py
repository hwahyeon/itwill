...
#expr = (X * a) + b
fir = tf.multiply(X, a, name='fir')
sec = tf.add(fir, b, name='sec')
...
with tf.Session() as sess :
    sess.run(init)
    print("X:", sess.run(X, feed_dict = {X : [[1, 2, 3]]}), sep = '\n')
    print('a:', sess.run(a), sep = '\n') 
    print('b:', sess.run(b), sep = '\n')
    
    print('expr:', sess.run(sec, feed_dict = {X : [[1, 2, 3]]}), sep = '\n')
