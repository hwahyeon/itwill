...
train_ds
# ((None 여기에 32가 들어가는 것, 784), (None,)), types: 타입도 변경됨(tf.float64, tf.uint8)>
...
class Model(tf.keras.Model): # 자식클래스(부모클래스)
    def __init__(self) : # 생성자
        ...
        self.d3 = Dense(10, activation='softmax') # output layer 마지막은 출력의 갯수와 맞춘다.
...
# 4. loss function : 손실 함수(오차 변환) -> losses.class 대체
loss = losses.SparseCategoricalCrossentropy(from_logits = True)
# y_ture(integer) vs y_pred(prop) : from_logits = True
...
def train_step(images, labels) :  
    with tf.GradientTape() as tape:
        # 1) 순방향 : loss 계산
        preds = model(images) # model.call(images)이것과 같은 의미.
        loss_value = loss(labels, preds) # 손실함수 호출(y_true, y_pred)
