...
print(x_train[0]) # 첫번째 이미지의 픽셀정보 0 ~ 255
...
# Filter 변수 정의 
Filter = tf.Variable(tf.random_normal([3,3,1,5])) # (row, column, color, fmap) 난수 
 
# 1. Convolution layer : 특징 추출
conv2d = tf.nn.conv2d(firstImg, Filter, strides=[1,1,1,1], padding='SAME')
print(conv2d) # Tensor("Conv2D:0", shape=(1, 28, 28, 5), dtype=float32)

# 2. Pool layer : down sampling
pool = tf.nn.max_pool(conv2d, ksize=[1,2,2,1],strides=[1,2,2,1],
            padding = 'SAME')
print(pool) # Tensor("MaxPool:0", shape=(1, 14, 14, 5), dtype=float32)
