...
# 합성곱 계층 함수 (중복코드를 함수로 정리하고 필요할 때마다 쓰면 됨.)
...
with tf.name_scope("Convolution1") as scope:
    ...
    # 함수 사용
    conv2 = tf.nn.conv2d(X_img, Filter1)
    
    L1 = tf.nn.relu(conv2) # 정규화 : 0 ~ x
    #함수 사용 L1_out = tf.nn.max_pool(L1, ksize = [1,2,2,1], strides=[1,2,2,1], padding='SAME')
    L1_out = max_pool(L1)
...
with tf.name_scope("Convolution2") as scope:
    ...
    # 함수 사용
    conv2_l2 = tf.nn.conv2d(L1_out, Filter2)
