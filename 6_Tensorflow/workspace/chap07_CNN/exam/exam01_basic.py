...
# Filter 변수 정의 
Filter = tf.Variable(tf.random_normal([3,3,1,10])) # (row, column, color, fmap)
 
# 1. Convolution layer : 특징 추출 
conv2d = tf.nn.conv2d(Img, Filter, strides=[1,2,2,1], padding='SAME')
print(conv2d) # Tensor("Conv2D:0", shape=(1, 14, 14, 10), dtype=float32)

# 2. Pool layer : down sampling
pool = tf.nn.max_pool(conv2d, ksize=[1,3,3,1],strides=[1,2,2,1],
            padding = 'SAME')
print(pool) # Tensor("MaxPool:0", shape=(1, 7, 7, 10), dtype=float32)
...
with tf.Session() as sess :
    ...
    print(conv2d_img.shape) # (1, 14, 14, 10) -> (10, 14, 14, 1)
    ...
    pool_img = np.swapaxes(pool_img, 0, 3) # (1, 7, 7, 10) -> (10, 7, 7, 1)
