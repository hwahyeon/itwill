...
# 37개가 분류대상이란 의미. 0의 갯수는 오분류라고 표현한다. 잘못 분류한 개수를 말함.
# setosa는 가장 많은 레이블을 말하며, 이를 '주 레이블'이라고 한다.
# 즉 오분류는 하나도 없고 37개가 분류되었으며, 가장 많은 레이블은 setosa이다.
# -> setosa가 37개이다.
# (1.00000000 0.00000000 0.00000000) 첫번째 레이블 100%, 나머지 두 종류는 없다는 뜻.
# left node : 분류조건 -> (37개 0개) 가장많은 비율 label (각 label 분류 비율) 단노드 유무  
# 3) Petal.Length>=2.45 68 33 virginica (0.00000000 0.48529412 0.51470588)  
# right node : 형태 -> 조건 분류대상 68개, 오분류 33개, 주label (각 label 분류 비율)
# (0.00000000 0.48529412 0.51470588)   가장 비율이 높은 51%가 virginica라는 뜻.
# 68개에서 33을 뺀 값이 바로 주분류인 virginica를 의미하는 것이다.
...
#Rpart는 과적합이 발생할 수 있다. -> 앙상블 기법을 사용해서 해결할 수 있다.
# 또 다른 해결책이 바로 가지치기이다.
####################################
## 가지치기(cp:cut prune)
####################################
# 트리의 가지치기 : 과적합 문제 해결
# cp : 0 ~ 1, default = 0.05
# 0으로 갈수록, 트리 구조가 커진다. 즉 깊이가 커진다. -> 깊을 수록 예측률이 높아지고 오류가 적어진다.
# 하지만 너무 깊으면 과적합의 문제가 생긴다. 오분류는 줄어들지만 과적합 문제가 는다.
# 따라서 cp의 값을 조금씩 조절해야한다. 최적의 cp값을 찾아야한다.
model$cptable
#   CP nsplit    rel error xerror       xstd
# 1 0.5147059      0 1.00000000 1.11764706 0.06737554
# 2 0.4558824      1 0.48529412 0.57352941 0.07281162
# 3 0.0100000      2 0.02941176 0.02941176 0.02059824 # 가장 작은 오류율
# -> cp가 0에 가까울수록 오류률이 적다.
# 과적합 발생 -> 0.45 조정
