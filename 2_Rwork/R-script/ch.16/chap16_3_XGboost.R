...
#Dim위치에 있는 것을 slot이라 함.
# $data : x 변수(slots) [6513, 126] : 2차원(matrix)
# $label : y변수 : num [1:6513] : 독성이 있냐 없냐 구분할 수 있는 변수//1차원(vector)

# class -> object
# Java, Python : object.member
# R : object@member #R에서는 .도 변수로 인식해버림.

str(test)
# $ data : x변수 : @ Dim     : int [1:2] 1611 '126' #(x변수와 y변수의 길이가 같아야함.)
# -> [1611, 126]
...
#  Feature(x변수)   Gain(y에 영향을 주는 정도)   Cover   Frequency
...
# 1. y변수 -> binary
iris_df$Species <- ifelse(iris_df$Species == 'setosa', 0, 1) #setosa를 0으로 바꾸고 나머진 1로 바꾸자
str(iris_df) #$ Species     : num  // 팩터형에서 num형으로 변경.
...
str(iris_df) #Species     : num 원래 팩터형인데 num형으로 변경된 것을 볼 수 있다.
table(iris_df$Species)
#  0  1  2 
# 50 50 50 
...
xgb_iris_model2 <- xgboost(data = dmtrix,
                           ...
                           num_class = 3, #multi니까 숫자를 명시해줘야함.
                           verbose = 0)
xgb_iris_model2

# 5. prediction
test_x = as.matrix(text[-5]) #matrix
test_y = test$Species #vector

# softmax : 0-1 = sum(1)이 되게끔 한다.
...
cat('accuracy =', acc) #0.9333333

mean_err <- mean(pred != test_y)
cat('mean error =', mean_err) #mean error = 0.0666667

# 학습된 model의 변수(feature)
import <- xgb.importance(colnmaes(train_x),
