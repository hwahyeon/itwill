...
max(Bug_Metrics_Software[,,1])
barplot(Bug_Metrics_Software[,,1], ylim = c(0,15000),
        main = "각 소프트웨어별 버그",beside = TRUE,
        col = rainbow(5))
#범례 추가
bug1<-Bug_Metrics_Software[,,1]
row_names <- row.names(bug1)
legend(x=25, y=15000, legend = row_names, fill=rainbow(5))


# 단계2) 소프트웨어 발표 후 버그 수를 대상으로 각 소프트웨어별 버그를
# 누적형 가로막대 차트로 시각화하기(각 막대별 색상적용) 
max(Bug_Metrics_Software[,,2])
barplot(Bug_Metrics_Software[,,2],horiz =TRUE, xlim = c(0,1500),
        main = "각 소프트웨어별 버그",
        col = rainbow(5))
legend(x=1000, y=5, legend = row_names, fill=rainbow(5))
...
plot(quakes$long,quakes$lat, col = quakes$stations, main = "지진의 진앙지 산점도 차트")
...
pairs(iris3[])
pairs(iris3[,,'Versicolor']) #pairs(iris3[,,2])와 같다.
