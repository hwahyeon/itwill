...
# 4) tm 설치 
install.packages("tm") 
#text mining의 약자. 텍스터 전처리, DTM 문서 단어 행렬, TDM 단어 문서 행렬을 만듦.

# 5) 패키지 로딩
library(rJava) 
library(KoNLP) #Sejong 자동 로딩
#Checking user defined dictionary!
#‘hash’이라고 불리는 패키지가 없습니다 -> 설치하면 됨.
...
#"나"     "김진성" "대한"   "민국"   "사람"  단어 벡터

paste(extractNoun("나는 김진성이고, 대한민국 사람이다."), collapse=" ")
#"나 김진성 대한 민국 사람" 하나의 string으로 묶여짐.
...
#문장형으로 변한하고 형태소 변환한 후 공백을 기준으로 합쳐지는 것.
...
# Maximal term length: 12 : 6음절이 가장 큰 음절
# Weighting          : term frequency (tf) : 단어의 출현빈도수


str(myCorpusPrepro_term) #<<TermDocumentMatrix (terms: 696, documents: 76)>>
...
wordResult[1:20]
word_name <- names(wordResult) #단어이름
word_name[1:10] #출현빈도
