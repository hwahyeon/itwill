...
# - lattice, latticeExtra(생략), ggplot2, ggmap 패키지
##########################################
# 1. lattice 패키지
#########################################
...
#Factor 원래 문자였는데 Factor로 변경된 것들일수있음.


# 1.histogram(~x축, dataframe)
histogram(~gcsescore, data=Chem97) 
# gcsescore변수를 대상으로 백분율 적용 히스토그램
# 각 막대의 아랫변 길이를 도수, 계급이라고 부름.
table(Chem97$score)
# 0    2    4    6    8   10 
# 3688 3627 4619 5739 6668 6681 
# score는 숫자변수지만 이것을 집단변수로 적용해서 사용할 수 있다.
#총 6가지 범주니까 |뒤에 사용하면, 6개의 격자에 그래프가 생기는 것.
...
#범주형으로 보여주고 싶을 때 factor로 감싸면 된다. score가 가지고 있는 요인의 값을 제목으로
#쓸 수 있는 것.
table(Chem97$score)
...
#barchart에서는 범위를 dataframe으로만 해줘야하기 때문에 행렬인 VADeaths를 dataframe으로 바꿔준 것.
...
# y축에 var1이, x축에 Freq이 들어가는 것. var2는 격자를 만들어주는 역할 -> 4개의 격자가 만들어짐.
...
#사용할 데이터는 quakes, 산점도의 모양은 .으로 하겠다는 의미.
# 그래프를 변수에 저장
...
# depth 이산형 변수 리코딩
# 1. depth변수 범위
range(quakes$depth)# depth 범위는 숫자의 의미를 가지고 있는 '연속형 변수' (수심의 깊이)
# 40   680
# 2. depth변수 리코딩 // 110정도 단위로 자름.
...
cloud(depth ~ lat * long , data=quakes,
      ...
      xlab="경도", ylab="위도", zlab="깊이")
#z축의 방향쪽으로 45도, x축의 반대방향으로 25도 틀어서 보이겠다는 뜻.
...
qplot(hwy, data=mpg, fill=drv) # fill 옵션 적용 // 히스토그램에 색상을 채우는 것

# binwidth 옵션 : 도수 폭 지정 속성
qplot(hwy, data=mpg, fill=drv, binwidth=2) # binwidth 옵션 적용 //도수의 폭을 조절
#도수의 폭을 넓히면 데이터 표현할 때 막대의 갯수가 줄어듦
...
qplot(wt, mpg, data=mtcars, color=carb) # 색 구분이 굉장히 힘듦.
#carb를 팩터형으로 바꿔서 색상 적용하는 것이 좋다.
...
str(diamonds)
# 주요 변수 
# price : 다이아몬드 가격($326~$18,823), carat :다이아몬드 무게 (0.2~5.01), //둘은 비례관계
...
qplot(wt, mpg, data=mtcars, geom=c("point", "line")) #산점도와 라인
...
#구글맵은 상대적으로 편하지만, key값이 필요함.

# 대구 중심 남쪽 대륙 지도 좌표 : 35.829355, 128.570088
# 대구 위도와 경도 기준으로 남한 대륙 지도(제주도까지 다 나오려면 대구를 중심으로 해야함.)  
...
# [단계1] dataset 가져오기
pop <- read.csv(file.choose()) #popultaion 2019-01
str(pop)
...
#tot_pot <- str_replace_all(pop$'총인구수', ",", "") #,를 공백으로 다 바꾼다는 뜻.
#문자열을 숫자형으로 변환하는 과정
tot_pop <- as.numeric(str_replace_all(pop$'총인구수', ",", "")) #값을 숫자로 변경
tot_pop
...
layer2 <- layer1+ geom_point(data = df, aes(x=lon, y=lat,
                          ...
                          size=factor(tot_pop))) #각각 지역의 총 인구수를 색깔로 삼겠다는 뜻.
layer2
...
# 지도 이미지 file save
ggsave("pop201901.png", scale = 1, width = 10.24, height = 10.24)

#구글은 키를 받아서 개인적으로 연습해볼 것.
