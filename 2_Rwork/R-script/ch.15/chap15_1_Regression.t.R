...
# t value(-1.96~+1.96), 이 범위를 벗어나면 기각력
# p-value가 0에 가까울수록 영향을 미치는 감도가 세다.
...
summary(model)
# education    177.199    187.632   0.944    0.347    (상관없음) //영향이 없는 것은 모델에서 빼버림
...
# MSE : 표준화 전  
mse <- mean(res**2) # 평균제곱오차 (오차에 제곱을 하면 오차값이 더 커지는 것을 이용)
cat('MSE =', mse) # MSE = 6369159 : 표준화 전 

# 잔차 표준화 
res_scale <- scale(res) # mean=0, sd=1
summary(res_scale) #표준화 확인
...
new_data <- Prestige[, c(1:5)] # 사용가능한 모든 것을 사용해 모델을 일단 만듦.
dim(new_data) # 102   5

model2 <- lm(income ~ ., data = new_data)

library(MASS)
step <- stepAIC(model2, direction = 'both') # stepAIC : x변수에 대한 최적의 모델을 선택해줌.

model3 <- lm(income ~ women + prestige, 
             data = new_data)               # 최적의 변수는 모델의 설명력과 상관이 있다.
summary(model3)
...
# - 생년월일과 나이를 독립변수로 갖는 경우 # 이런 것은 다중공선성 이유로 같이 사용하면 안됨. 신뢰성이 떨어짐.
# - 해결방안 : 강한 상관관계를 갖는 독립변수 제거

# (1) 다중공선성 문제 확인 (독립변수가 2개 이상인 다중회귀분석에서 나올 수 있는 문제, 단순회귀에선 불가.)
library(car)
...
# FALSE         TRUE         TRUE 
#TRUE끼리 다중공선성의 문제가 의심되는 것임.

#Sepal.Length가 y의 역할 x1, x2의 값들로 학습하는 것임.
