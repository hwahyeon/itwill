...
# 단계1 : 트랜잭션 객체 생성 및 확인
btran <- read.transactions("tranExam.csv", 
                           format="single", 
                           sep=",", 
                           cols=c(1,2), 
                           rm.duplicates=T)
...
arule <- apriori(btran, parameter=list(supp=0.3, conf=0.1)) 
arule # set of 16 rules
...
race <- subset(rules, lhs %in% 'race=White')
race # 46 rules
plot(race, method='graph')
