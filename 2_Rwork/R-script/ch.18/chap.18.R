...
#매트릭스 한 행을 포인트로 두고 행끼리의 거리를 구함.

#    1(첫번째 관측치) 2(두번째 관측치)
# 2  5.196152          
# 3 10.392305          5.196152
# 즉 첫번째 관측치와 두번째 관측치의 유클리드 거리는 5.196152라는 뜻.
# dist함수는 하삼각모양을 그린다.
# 1포인트와 3포인트의 거리가 가장 멀다고 볼 수 있다.
...
# (2) 데이터 셋 생성
r <- runif(15, min = 1, max = 50) #난수 생성 1에서 50 사이
...
# 거리가 가까운 것들을 군집으로 형성해서 그린 것.

#유클리드 계산식으로 거리가 짧은 것들끼리 묶어나가는 과정으로
#클러스터를 만든다.
...
# 즉 150개를 1~3 숫자로 커팅해서 나눠줌.

# 4) iris에서 ghc 컬럼 추가
iris$ghc <- ghc
table(iris$ghc) # ghc 빈도수
head(iris,60) # ghc 칼럼 확인 (60개까지만 가져옴)
...
# "cluster" 클러스트 정보
#"centers" 중앙값      "totss"        "withinss"     "tot.withinss"
...
plot(mydia$carat, mydia$price, col=mydia$cluster) #색깔
...
# 도움을 얻는 용도이지 맹신할만한 방법은 아니다!
# 직접 클러스트를 번갈아가며 실행해봐야함.
