...
shapiro.test(z)
# W = 0.99805, p-value = 0.3048

hist(z, freq = F)

# 1) 표준화 공식 (Z) = (X-mu) /sigma
mu=mean(z)
z=(z-mu)/sd(z) #정규 -> 표준정규
mean(z) # -5.247784e-16
sd(z) # 1
...
# 표준화 : A학생 90점, B학생 90점 둘이 본 시험의 난이도가 다름.
# 이 둘의 레벨을 비교하기 위해 '척도'를 일치시키는 행위
...
nor <- function(x){ # 함수의 반환값 0~1 사이로 정규화함.
...
#two.sided양측검정, conf.level=0.95 이 둘은 기본값으로 안써줘도 된다.
...
#p-value = 0.0003179 < 0.05 채택 : 양측검정과 반대이다.
# -> 유의미하다는 표현을 사용함.
...
# p-value = 0.0003422 < 0.05
# 귀무가설 채택/기각할 것인지에 대한 검정에선 같은 결과가 나옴.
...
# c(50,50,50) // 두번째 집단은 시행횟수
#한집단이면 binom.test 두집단 이상이면 prop.test를 쓴다.
# p-value = 0.5232 >= 0.05
...
# [해석] 적어도 한 집단에선 평균의 차이를 보인다.
# p-value에 대한 강도를 *로 나타냄.
# p-value *** = 0에 근사하는 값일 경우. *의 수가 많을 수록 매우 유의미하다는 뜻.
# [해설] 매우 유의미한 수준에서 적어도 한 집단에서 평균에 차이를 보인다.
# F-value와 p-value를 보고 해설할 수 있다.

# 사후검정
TukeyHSD(result) # 분석분석의 결과로 사후검정 95%신뢰수준
...
#[사후검정에서 해석] 세 집단 모두 집단의 차이를 보인다.


plot(TukeyHSD(result))
#[해석]
#신뢰구간에 0을 포함하면 집단간 차이 없음
#신뢰구간에 0을 포함하지 않으면 집단간 차이 있음
...
summary(result)
#             Df Sum Sq Mean Sq F value Pr(>F)
# age2         1    4.8   4.812    0.69  0.408
# time         1    0.0   0.001    0.00  0.990

# F value가 -1.96 ~ +1.96 안에 들어옴
