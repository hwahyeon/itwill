...
#data.frame 정렬 : 특정(cost) 기준
head(data[sort(data$cost),],30) #index 정렬 (vector 정렬용)

head(data[order(data$cost),],30) #내용 정렬 (data.frame 정렬용)

library(dplyr)
arrange(data, cost) #오름차순 // 새로운 행의 이름이 만들어짐.

# order vs arrange
#order는 기존 행 번호 유지
#arrange는 새로운 행 번호 적용

# 최빈수(mode)
mode() # data type을 제공하는 함수
mode(x) #"numeric"
...
# 최빈수=중위권=평균 : 좌우 대칭
# 최빈수<중위권:평균 : 단봉 - 오른쪽 기울어짐
# 최빈수:중위권<평균 : 단봉 - 왼쪽 기울어짐 = 오른쪽에 비대칭 꼬리가 만들어진다.
# 봉이 여러가지 있을 때는 위 규칙이 틀릴 수 있다.
...
kurtosis(cost)    # 2.674163 < 3  표준정규분포가 3정도 값을 가짐
...
hist(cost, freq = F)
#freq가 false로 바뀌면서 density로 변경됨.

# 확률밀도 분포 곡선 : 히스토그램의 밀도 추정 
lines(density(cost), col='blue') # cost에 대한 확률밀도분포 곡선
