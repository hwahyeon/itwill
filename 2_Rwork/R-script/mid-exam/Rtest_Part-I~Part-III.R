...
clean_election2 <- subset(election, contbr_occupation != "NA")
clean_election2

# [문제3] 5만개 관측치만 샘플링하여 clean_election2 변수에 저장하시오.
#  힌트) sample() 함수 이용
clean_election2 <- sample(50000)
head(clean_election2)

# [문제4] 'Romney, Mitt'와 'Obama, Barack' 후보자별 서브셋을 작성하여 romney, obama 변수에 저장하시오
romney <- subset(election, cand_nm == 'Romney, Mitt')
obama <- subset(election, cand_nm == 'Obama, Barack')
...
obama_romney2 <- subset(obama_romney$contbr_occupation2, obama_romney$contbr_occupation2 != "NA")
obama_romney2

# <조건4> obama_romney2 변수를 대상으로 cand_nm 칼럼이 'Obama, Barack'이면 '당선', 
#               'Romney, Mitt'이면 '낙선'으로 파생변수를 생성하여 cand_pass 칼럼 추가
library(dplyr)
obama_romney2$cand_pass <- obama_romney2 %>% mutate(ifelse(cand_nm >='Obama, Barack', '당선', '낙선')) %>% obama_romney2$cand_pass
...
table(obama_romney$contbr_occupation2, obama_romney2$cand_pass)
...
obama_romney3 <- contb_receipt_amt >

election$contb_receipt_amt
  
densityplot(~contb_receipt_amt|cand_pass == '당선', groups = contbr_occupation, plot.points=F, auto.key = T)
