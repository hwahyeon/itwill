...
data = Dataset(reader)
dataset = data.load_from_df(ratings[['critic', 'title', 'rating']],
                              reader)
...
pred
# uid='Jack', iid='Just My', r_ui=3.225806451612903, est=3.1400592052665344, details={'was_impossible': False}
# uid=사용자, iid=영화, r_ui=실제 평점, est=추정예측치 평점

# uid='Toby'
'''
uid='Toby', iid='Lady', r_ui=3.225806451612903, est=3.249434565608724
uid='Toby', iid='The Night', r_ui=3.225806451612903, est=3.294843268061017
uid='Toby', iid='Just My', r_ui=3.225806451612903, est=3.077712847792272
est가 가장 높은 The Night가 추천 대상이 된다.
'''

# 4. 개별 사용자 대상 예측치
user_id = 'Toby' # 추천 대상자
items_id = ['The Night', 'Just My', 'Lady'] # 미 관람 영화 // 추천 대상 아이템
actual_rating = 0 # 실제 평점

#test : 전체 사용자 예측치, predict : 특정 사용자 예측치

for item in items_id :
    print(model.predict(user_id, item, actual_rating))

'''
user: Toby       item: The Night  r_ui = 0.00   est = 3.29   {'was_impossible': False}
user: Toby       item: Just My    r_ui = 0.00   est = 3.08   {'was_impossible': False}
user: Toby       item: Lady       r_ui = 0.00   est = 3.25   {'was_impossible': False}
'''
