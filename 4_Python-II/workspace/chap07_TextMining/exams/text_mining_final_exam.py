...
type(movies.genres) #pandas.core.series.Series
...
zarr = np.zeros((len(movies),len(np.unique(mgl))))
print(zarr)
type(zarr) #numpy.ndarray
...
mov_df = pd.DataFrame(zarr, columns = uni)
mov_df
...
movie_count = mov_dft.sum(axis=1) 
print(movie_count)
...
for n, g in enumerate(movies.genres) : 
    dummy.loc[n, g.split("|")] = 1  # Animation|Children's|Comedy

'''
gen = "Animation|Children's|Comedy"
gen.split('|') # ['Animation', "Children's", 'Comedy']
dummy.loc[1, ['Animation', "Children's", 'Comedy']]
'''
