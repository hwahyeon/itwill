...
[2문] 다음 벡터(pay)는 '입사년도사원명급여'순으로 사원의 정보가 기록된 데이터 있다.
 ...
 유관순 => 260 
'''
from re import findall, match, sub, split
pay = ["2014홍길동220", "2002이순신300", "2010유관순260"]

a = []
for i in pay:
    a.append(int((findall('[0-9]{3}$', i)[0])))
print('전체 급여 평균 :', mean(a))
print('평균 이상 급여 수령자')
for i in pay:
    if int((findall('[0-9]{3}$', i)[0])) >= mean(a) :
        print(findall('[가-힣]{3}', i)[0],'=>',int((findall('[0-9]{3}$', i)[0])))
...
def students():
    ...
    def score_display(student):
        ...
        for i in list(range(0, 3)):
            print(' {}\t\t{}\t\t{}\t\t{}\t\t{}\t\t{}'.format(i + 1, student[i][0], student[i][1], student[i][2],
                                                                 sum(student[i]), mean(student[i])))
    return score_display

a = students()
a(student)

print("원주율=", format(321.14159, "2.2f") ) # 전체자릿수.소수점자릿수
